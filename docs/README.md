...
### Serverless Event Channels
* Event channels are simply a named queue for events.
* When a generator emits an event it is pushed to the channel's queue.
* Channels are isolated through a combination of the `service` and `stage`
* A channel can be opened to a random server that is not part of the Serverless Platform

##### Example: Generating a channel using `service` and `stage`  
```js
import { channel } from 'fdk'
...
##### Example: Generating a channel using a url  
```js
import { channel } from 'fdk'

const bobsChannel = channel({
  url: "https://bobsservice.com/some/path"
})
```


#### Emitting a Serverless Event
* Events can be emitted from anywhere using the Serverless FDK.
